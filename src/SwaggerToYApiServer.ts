...
export class SwaggerToYApiServer {
    ...
    async getYApiData(): Promise<
        AsyncReturnType<typeof swaggerJsonToYApiData>
    > {
        if (isEmpty(this.yapiData)) {
            this.yapiData = await swaggerJsonToYApiData(
                await this.getSwaggerJson()
            );
        }
            ...
            this.httpServer = http
                .createServer(async (req, res) => {
                    ...
                    if (pathname!.includes("/api/plugin/export")) {
                        ...
                        pathname!.includes("/api/interface/getCatMenu")
                    ) {
                        res.end(
                            JSON.stringify({
                                errcode: 0,
